...
- **Issue:** The Task 1-approved policy deliberately left `numpy`/`scipy`/`scikit-learn`/`faiss-cpu` unpinned in `requirements-notebooks.txt` (they're already core `effdim` deps). The user runs notebooks in their own pre-existing venv and wants the file to be self-sufficient — a direct reversal of that policy, not a defect in what was built.
- **Fix:** Added `numpy==2.5.1`, `scipy==1.18.0`, `scikit-learn==1.9.0`, `faiss-cpu==1.14.3` (pinned to the exact versions the tracer ran under and the notebook's own §0.3 header prints), plus `joblib==1.5.3` (direct import in `cache.py`, previously only transitive) and `pytest==9.1.1` (from `pyproject.toml`'s `dev` extra, pytest only). Rewrote the file's header comment to state the new "fully provision a user-supplied venv" policy and to clarify D-14's cache-key contract is unaffected (it hashes installed versions at runtime, not this file's pins).
- **Files modified:** `notebooks/requirements-notebooks.txt`
- **Verification:** Plan's automated `<verify>` block re-run; the original assertion that these four packages are *absent* from the file is superseded (documented here, not silently weakened) — all other checks (`trust_remote_code`, `eigvalsh`, `pyproject.toml`/`src/effdim/` untouched, 14/14 pytest, cache-key contract, notebook §0 assertions) still pass unchanged. No notebook re-execution was needed: the new pins match the versions already installed and already exercised by the committed outputs.
- **Committed in:** `c83045a` (separate atomic commit, as instructed)

---

**Total deviations:** 1 user-directed deviation (requirements-notebooks.txt full provisioning), plus one internal cache-key-scoping refinement that was already anticipated in the plan's own "Cache-key refinement" section (not counted as a deviation from the plan, since the plan itself names it as a deliberate, stated refinement).
**Impact on plan:** No scope creep. The requirements-notebooks.txt change is additive (more pins, same three original packages unchanged) and does not alter any code path, cache contract, or test. All plan `<verify>` checks pass except the one superseded assertion, which is explicitly documented rather than silently dropped.

## Issues Encountered

- **Provider-quota interruption:** A prior executor was cut off mid-Task-3 with zero commits, leaving `requirements-notebooks.txt` and `cache.py` untracked. Both were reviewed against the plan spec on resume and found correct; no rework was needed.
- **Stray kernel corruption during the deviation fix:** Between committing `e584180` and applying the coordinator's requested change, a stray `ipykernel` process (system Python 3.13.14, not the project `.venv`, likely from an IDE/editor auto-opening the notebook) re-executed `01_manifold_and_gate.ipynb` in place and overwrote its real outputs with a broken run (`ModuleNotFoundError: No module named 'scipy'`). Detected via `git diff` before staging, the stray process was killed, and the notebook was restored via `git checkout -- notebooks/01_manifold_and_gate.ipynb` (a targeted single-file restore per the destructive-git-prohibition's sanctioned exception, not a blanket reset) back to the verified, committed good state. No data was lost since the file had already been committed in `e584180`; the corruption only touched the uncommitted working tree.
