...
`STAGE2_K = [5, 10, 15, 30]` is unevenly spaced (gaps 5, 5, 15). The connected values k=8 and k=20 were never fit because `STAGE2_MAX_FITS=4` dropped them (evenly-spaced-with-endpoints selection over the 6 connected base-range values). The plateau run `[10, 15, 30]` is therefore maximal in *index* space, not in *k* space -- the 15→30 adjacency carries the same weight as 10→15 despite spanning three times the range. The plateau is real but coarse-grained. The user was shown this and chose to apply the pre-registered rule unchanged rather than fit the two skipped values, on the grounds that reopening a frozen sweep parameter (`STAGE2_MAX_FITS`, or refitting k=8/k=20) after seeing sweep results is exactly the garden-of-forking-paths failure §4.0's pre-registration exists to prevent. This is a documented gap in the plateau evidence's granularity, not a defect in the computation -- carried forward here for Phase 2 and any future planner to see, per instruction, without triggering rework in this plan.
