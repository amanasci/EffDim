...
**`fit_key` coincidence with the plan 03 sweep artifact, explained so a future reader does not mistake it for a bug.** `fit_key=43cf438bc944c509` is identical to plan 01-03's `sweep_k15_43cf438bc944c509.npz` key. This is correct cache-contract behaviour: `config_key` hashes `json.dumps(cfg, sort_keys=True)`, and once `ANALYSIS_CFG["n_neighbors"]` was set to `15` in §5.1, `ANALYSIS_CFG` and the stage-2 sweep's per-k `fit_cfg` for `k=15` became field-for-field identical D-14 config dicts -- identical inputs hash to identical keys, by design. The two artifacts are **not** the same file: `sweep_k15_43cf438bc944c509.npz` (~1.07 MB slim record, no `dist_matrix_`, written in plan 03) and `isomap_43cf438bc944c509.joblib` (~1.55 GiB, carries `dist_matrix_`, written in this plan) are distinct files under distinct stems (`sweep_k{k}_...` vs `isomap_...`) that merely share a 16-hex suffix.
