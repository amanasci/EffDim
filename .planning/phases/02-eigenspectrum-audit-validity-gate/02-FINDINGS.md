...
This document reports two experiments. It is written for a reviewer with no prior context on
...
work that assumed a valid flat embedding cannot proceed on that assumption.
...
**Note for interpretation (§6):** rows are L2-normalized, so the points lie on the unit
hypersphere in R^768.
...
- The residual elbow (5) sits far below the prior phase's provisional intrinsic-dimension
  estimate of 18 (median over eight geometric estimators). On a gate that has already failed we
  read this as another symptom rather than an independent finding, but it is a 3.6× discrepancy
  and worth a reviewer's attention.
...
## 6. Interpretation — what is established, and what is not
...
5. Therefore **classical MDS is not a valid description of this geodesic metric**, and any
   downstream method assuming a valid flat Isomap embedding is unsupported on this fit.

### Inferred, not established

**That the cause is intrinsic manifold curvature (H1).** H2 (hop inflation) is disfavoured by
the evidence above, and H1 is the natural remaining candidate — but these experiments were
designed to discriminate H1 from H2, not to establish H1 positively against all alternatives.
A reviewer should treat "intrinsic curvature" as the surviving hypothesis, not a demonstrated
conclusion.

### Alternative explanation not disentangled — flagged explicitly

**The embeddings are L2-normalized, so the points lie on the unit hypersphere in R^768.** A
hypersphere's geodesic metric is genuinely non-Euclidean, and classical MDS on spherical
geodesics produces negative eigenvalues *by construction* — independent of any interesting
astronomical structure in the data.

These experiments did not separate "curvature of a scientifically meaningful low-dimensional
manifold" from "curvature of the normalization sphere the data was projected onto." That
distinction matters a great deal for what the result means, and it is not resolved here. A
reviewer should regard it as the most important open question raised by this work. An obvious
next test — an unnormalized or differently-normalized fit — was not run.
...
- **One dataset config.** One of 163 configs in the source dataset.
...
- **Normalization confound unresolved** (§6).
- **Fixed `n_components = 18`** across all fits. This does not affect `r`/`m`, which derive from
  the full spectrum, but it does constrain the embeddings.
