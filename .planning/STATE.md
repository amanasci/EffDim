...
last_updated: "2026-09-03T04:54:18.187Z"
...
| Phase 07 P02 | 27min | 2 tasks | 3 files |
...
- [Phase ?]: Tracer feedback gate confirmed (ratify: proceed to Task 2); smoke verdict ASSOCIATION DETECTED understood by all parties as a stubbed smoke-path artifact, not a Phase 7 finding
- [Phase ?]: per_point_mknn, two_tailed_permutation_null, apply_verdict, split_indices pinned by 22 regression tests (140 total), including the explicit test documenting the closed one-sided-permutation defect
...
- [Phase ?]: 09-03: --mode manifest CLI/record contract (row_kind key, comma-joined --candidate-columns) matched exactly to 09-04-PLAN.md's own hardcoded invocation, discovered by reading ahead
- [Phase ?]: 09-03: full-scale --mode manifest exercised live against real HuggingFace data (25m47s wallclock); mass_med_photoz reproduces the colleague's reported 79,490/86,471 exactly
...
- Phase 8 added 2026-08-27: Curvature-Conditioned CKA Alignment. Adds CKA as a second alignment probe to test whether Phase 7's curvature-MKNN association is MKNN-specific. **D7-07 declared CKA out of scope for Phase 7 as "a separate decision, not a Phase 7 task" — this is that decision, taken by the developer 2026-08-27; no sealed verdict is reopened.** Design chosen from four options: **global CKA, curvature-conditioned** — split points by `||H||` magnitude into density-matched, equal-n subsets, compute CKA on each, permutation-null the difference. Reuses Phases 5/6's magnitude-split pattern (Phase 5 already split on `||H||` magnitude rather than Phase 4's direction sign), NOT 07.1's per-point partial design. Reason: `mknn.mknn_score` yields an `(n,)` per-point array (D7-04) which is what makes 07.1's partial correlation possible; standard CKA is a single global scalar per dataset, so 07.1's partial/stratified-null machinery has nothing to attach to. A per-point local-CKA variant was considered and not chosen (unsealed instrument, small-sample CKA bias, full validation burden).
- Phase 8 design constraints, measured 2026-08-27 before planning: `spearman(density, ||H||)` on the frozen Phase 7 fields is **+0.4281 at d=20, +0.3150 at d=25, +0.0118 (p=0.238, nil) at d=32**; `spearman(density, MKNN) = -0.2121` at every d. Phase 4's `-0.0273` does NOT transfer to the v1.1 fields, so at d=20/25 a curvature-magnitude split IS partly a density split and Phase 4's Gap-3 confound reappears through a different door. Consequences: (1) density-matched subset construction is load-bearing, not a robustness check; (2) equal-n subsets are mandatory because CKA is biased upward on small samples — the analogue of Phase 4's `k/n_region` chance-floor artifact that inflated region 1's raw MKNN. Proposed: run all three d and treat **d=32 as a built-in negative control for the split machinery** — it has a clean curvature/density split AND no surviving association from 07.1, so a gap appearing there is evidence the split manufactures it. CKA does not exist anywhere in the codebase and needs its own low-`d` anchor and planted-effect positive control before any PU number counts.
...
Last session: 2026-09-03T04:54:18.126Z
