...
| 260815-e1t | Part A: spiked a first-order isometry/conformal prior on the CAE chart decoder's Jacobian (`decoder_priors.py`, opt-in, `cae.py` untouched, anchor `rho_chart=-0.06041003026778113` reproduced exactly). The weight-ladder run **halted at its own pre-declared `--probe` budget gate** (`BUDGET NOT MET`) before training any cell — the prior triples per-epoch cost and even the cheapest candidate (E=50) projects 3636s against a 3000s budget; no mechanism/bias verdict exists. Part B: retired D-12's escalation trigger — it FIRED on both legs of the corrected PU grid (`TRIGGER FIRES = True`), recorded in full, retired because the CAE-vs-plain-AE comparison is entirely C0 and cannot bound C2/curvature error, not because of the unfavourable result. Replaced with a two-part C0 (proposed `mse_per_dim < 2.5e-04`, awaiting ratification) / C2 (`ROLL_FLOOR=0.65`) criterion. Ends on a blocking checkpoint: four Part-A decisions plus C0 threshold ratification unresolved | 2026-08-15 | 762f7be + 8930b15 + 7ccdec7 + 505e890 | [260815-e1t-isometry-prior-spike-and-retire-cae-vs-p](./quick/260815-e1t-isometry-prior-spike-and-retire-cae-vs-p/) |
