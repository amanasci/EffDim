...
Estimates the dimension by covering the set with boxes of size $\epsilon$ and counting the number of boxes $N(\epsilon)$ required. The dimension is defined as:

$$ d = \lim_{\epsilon \to 0} \frac{\ln N(\epsilon)}{\ln (1/\epsilon)} $$

In practice, this is estimated by the slope of the linear regression of $\ln N(\epsilon)$ vs $\ln (1/\epsilon)$ for various box sizes.
